...
This appendix documents the full battery of sensitivity tests, bootstrap procedures, and alternative specifications. The main text highlights the text quality confound; here I present everything else.

### Bootstrap Confidence Intervals: Technical Details {#sec-appendix-bootstrap}
...
The bootstrap means (0.301 for capacity, 0.199 for ethics) match point estimates within rounding error, confirming that the resampling procedure recovers population parameters accurately.

### Cluster Stability: Silhouette Analysis Details {#sec-appendix-cluster-stability}
...
#### Specification 1: Excluding International Organizations
...
Excluding international organizations produces negligible changes: the capacity gap declines from d = 0.30 to d = 0.29 (3% reduction), remaining statistically significant. International organizations are not driving the observed income-group patterns.

#### Specification 2: Ordinal Regression
...
The ordinal logit model yields virtually identical statistical significance (z = −4.21 vs t = −4.26) despite different coefficient scales (log-odds vs linear). The proportional odds assumption holds (Brant test: χ² = 18.3, df = 12, p = .11). Results are robust to functional form.

#### Specification 3: Winsorizing Extreme Scores
...
#### Specification 4: Alternative Income Classifications
...
#### Specification 5: Alternative Text Quality Thresholds
...
#### Specification 6: Temporal Subsamples
...
Income gaps remain significant across both periods but show slight attenuation over time (capacity d declines from 0.34 to 0.27, ethics d from 0.24 to 0.16), consistent with the convergence dynamics in Chapters 8 and 12. GDP effects remain weak and significant for capacity, near-zero for ethics. Core findings are temporally stable.

### Measurement Validation: Score Distributions
...
All dimensions show positive skewness (most policies score low) and substantial floor effects (23–54% score exactly zero). Ceiling effects are negligible (<1% for dimensions, 0% for composites), indicating adequate headroom in the 0–4 scale. Kurtosis stays within acceptable ranges (<3 for all composites), so no pathological tail behaviour threatens the parametric analyses.
